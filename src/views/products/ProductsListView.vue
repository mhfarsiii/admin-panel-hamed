<template>
  <div class="products-list-view text-black dark:text-gray-100 animate-fade-in">
    <!-- Header -->
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8 gap-4 lg:gap-0">
      <div class="flex items-center gap-4">
        <div class="w-12 h-12 bg-gradient-to-br from-primary-500 to-primary-700 dark:from-primary-600 dark:to-primary-800 rounded-2xl flex items-center justify-center shadow-lg shadow-primary-500/30">
          <svg class="w-6 h-6 text-black dark:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
          </svg>
        </div>
        <div>
          <h1 class="text-[14px] lg:text-3xl font-bold text-gray-900 dark:text-gray-100">مدیریت محصولات</h1>
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div>
          ...
          <select
            ...
            class="w-full px-4 py-3 border text-[12px] lg:text-[15px] border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all shadow-sm hover:shadow-md"
        ...
        <div>
          ...
          <select
            ...
            class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-[12px] lg:text-[15px] rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all shadow-sm hover:shadow-md"
            @change="fetchProducts"
          >
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">نوع محصول</label>
          <select
    ...
    <!-- Loading State -->
    <div v-if="productsStore.isLoading && products.length === 0" class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200/50 dark:border-gray-700/50 p-8">
    ...
    <!-- Products Table -->
    <div v-else-if="products.length > 0" class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200/50 dark:border-gray-700/50 overflow-hidden">
      <div class="overflow-x-auto">
        <table class="w-full">
          <thead class="bg-gradient-to-r from-gray-50 to-gray-100/50 dark:from-gray-700 dark:to-gray-800/50 border-b border-gray-200 dark:border-gray-700">
            <tr>
          ...
          <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200/50 dark:divide-gray-700/50">
            <tr v-for="product in products" :key="product.id" class="hover:bg-gradient-to-l hover:from-primary-50/30 dark:hover:from-primary-900/30 hover:to-transparent transition-all duration-200 group">
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="relative">
                  ...
                  />
                  <div v-else class="w-16 h-16 bg-gradient-to-br from-gray-200 to-gray-300 dark:from-gray-700 dark:to-gray-600 rounded-xl flex items-center justify-center">
              ...
              <td class="px-6 py-4">
                ...
                <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ product.slug }}</div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                ...
                <div v-if="product.discount > 0" class="text-xs font-semibold text-green-600 dark:text-green-400 mt-1 px-2 py-0.5 bg-green-50 dark:bg-green-900/30 rounded-lg inline-block">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <span
                  :class="{
                    'text-green-700 dark:text-green-300 bg-green-50 dark:bg-green-900/30': product.stock > 10,
                    'text-yellow-700 dark:text-yellow-300 bg-yellow-50 dark:bg-yellow-900/30': product.stock > 0 && product.stock <= 10,
                    'text-red-700 dark:text-red-300 bg-red-50 dark:bg-red-900/30': product.stock === 0,
                  }"
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex flex-col gap-1.5">
                  <span
                    :class="product.isActive ? 'bg-gradient-to-r from-green-100 to-green-50 dark:from-green-900/40 dark:to-green-800/30 text-green-800 dark:text-green-300 border border-green-200 dark:border-green-700' : 'bg-gradient-to-r from-red-100 to-red-50 dark:from-red-900/40 dark:to-red-800/30 text-red-800 dark:text-red-300 border border-red-200 dark:border-red-700'"
                  ...
                  <span
                    ...
                    class="px-3 py-1 bg-gradient-to-r from-blue-100 to-blue-50 dark:from-blue-900/40 dark:to-blue-800/30 text-blue-800 dark:text-blue-300 text-xs font-semibold rounded-full inline-block w-fit border border-blue-200 dark:border-blue-700"
                  >
                    ⭐ ویژه
                  </span>
                </div>
      ...
      <!-- Pagination -->
      <div v-if="meta" class="px-6 py-4 border-t border-gray-200/50 dark:border-gray-700/50 bg-gradient-to-r from-gray-50/50 dark:from-gray-800/50 to-transparent flex items-center justify-between">
        ...
        <div class="flex gap-2">
          <button
            ...
            :class="meta.page === 1 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-primary-50 dark:hover:bg-primary-900/30 hover:border-primary-300 dark:hover:border-primary-600 hover:text-primary-700 dark:hover:text-primary-400'"
          ...
          <button
            ...
            :class="meta.page === meta.totalPages ? 'opacity-50 cursor-not-allowed' : 'hover:bg-primary-50 dark:hover:bg-primary-900/30 hover:border-primary-300 dark:hover:border-primary-600 hover:text-primary-700 dark:hover:text-primary-400'"
            class="px-4 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-xl transition-all duration-200 font-medium shadow-sm hover:shadow-md"
          >
            بعدی →
          </button>
    ...
    <div v-else class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200/50 dark:border-gray-700/50 p-12 text-center">
      ...
      <p class="text-gray-600 dark:text-gray-400 text-lg font-medium mb-2">محصولی یافت نشد</p>
      <p class="text-gray-500 dark:text-gray-500 text-sm mb-6">شروع کنید و اولین محصول خود را اضافه کنید</p>
      <router-link
        to="/products/create"
        class="inline-flex items-center gap-2 px-6 py-3 bg-blue-500 text-white rounded-xl hover:from-primary-700 hover:to-primary-800 transition-all duration-200 font-semibold shadow-lg shadow-primary-500/30 hover:shadow-xl hover:shadow-primary-500/40 hover:-translate-y-0.5"
      >
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
        </svg>
    ...
    <BaseModal :modelValue="showDeleteModal" v-if="showDeleteModal" @close="showDeleteModal = false">
      <template #header>
        <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">حذف محصول</h3>
      </template>
      <template #body>
        <p class="text-gray-600 dark:text-gray-400">آیا از حذف محصول "{{ productToDelete?.name }}" اطمینان دارید؟</p>
